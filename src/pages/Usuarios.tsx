...
import { Users, Plus, Key, Shield, User, Loader2, AlertCircle, Edit2, Save, X, Power, PowerOff } from "lucide-react"
...
export default function Usuarios() {
  ...
  const [novaRole, setNovaRole] = useState<"user" | "admin">("user")
  ...
  // Função para desativar/ativar usuário (em vez de excluir)
  async function toggleUsuarioAtivo(id: number, username: string, ativoAtual: boolean) {
    ...
    if (confirm(`Tem certeza que deseja ${acao} o usuário "${username}"?\n\n${ativoAtual ? 'O usuário não poderá mais fazer login, mas seus consertos permanecerão no sistema.' : 'O usuário poderá voltar a fazer login normalmente.'}`)) {
      setLoading(true)
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="max-w-5xl mx-auto">
            ...
            <div className="p-5">
              <form onSubmit={cadastrarUsuario} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={novaRole}
                    onChange={(e) => setNovaRole(e.target.value as "user" | "admin")}
                    disabled={!isAdmin}
                    className="w-full px-3 py-1.5 text-xs border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition bg-gray-50 dark:bg-gray-900 focus:bg-white dark:focus:bg-gray-800 text-gray-900 dark:text-white disabled:opacity-50"
                  >
                  ...
                  >
                    ...
                    {usuarios.map(user => (
                      <option key={user.id} value={user.id}>
                        {user.username} {user.role === "admin" && "(Admin)"} {!user.ativo && "(Inativo)"}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {usuarios.map((user) => (
                  <tr key={user.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700 transition ${!user.ativo ? 'opacity-60' : ''}`}>
                    ...
                    <td className="p-3 text-center">
                      {isAdmin && user.username !== userLogado?.username && (
                        <button
        ...
        {/* Informação adicional sobre status */}
        {isAdmin && (
          <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <p className="text-xs text-blue-600 dark:text-blue-400">
              ℹ️ <strong>Gerenciamento de Usuários:</strong><br />
              • Usuários <strong>inativos</strong> não podem fazer login, mas seus consertos permanecem no sistema.<br />
              • A comissão padrão para novos técnicos é <strong>10%</strong>.<br />
              • Para <strong>reativar</strong> um usuário inativo, clique no botão verde ao lado do nome.
            </p>
          </div>
        )}
      </div>
