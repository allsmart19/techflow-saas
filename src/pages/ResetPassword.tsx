...
export default function ResetPassword() {
  ...
  useEffect(() => {
    // Verificar se o usuário está autenticado via token na URL
    const checkSession = async () => {
      try {
        // O Supabase já processa o token automaticamente
        // Só precisamos verificar se há uma sessão ativa
        ...
        if (error) {
          ...
          // Se não tem sessão, pode ser que o token ainda não foi processado
          // Aguardamos um pouco e tentamos novamente
          setTimeout(async () => {
  ...
  // Se houver mensagem de erro, mostrar tela de erro
  if (mensagem && mensagem.tipo === "error" && !newPassword) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-300 dark:from-gray-800 dark:to-gray-900">
        <div className="bg-white dark:bg-gray-800 w-full max-w-md p-8 rounded-2xl shadow-xl">
          <div className="text-center">
